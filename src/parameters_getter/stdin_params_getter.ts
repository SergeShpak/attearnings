        ...
        return new Promise<Parameters>((resolve, reject) => {
            ...
            rl.on('close', () => {
                ...
                if (groups.length !== summary.groupsCount) {
                    return reject(new Error(`expected number of groups: ${summary.groupsCount}, got: ${groups.length}`));
                }
                return resolve({
...
async function* getGroup(groups: number[]): AsyncIterator<number, Error | null, void> {
