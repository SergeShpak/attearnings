...
function readFilePart(path: string, limit: number, offset: number): Promise<readFilePartResult> {
        ...
        fs.open(path, 'r', (err, fd) => {
            ...
            fs.fstat(fd, (err, stats) => {
                ...
                if (bytesRead !== lenToRead) {
                    return reject(`expected to read ${lenToRead} bytes, actually read ${bytesRead}`);
                }
                return resolve({
                    buf,
...
async function* getGroup(file: string, offset: number): AsyncIterator<number, Error | null, void> {
