...
import {
  ParamsSource,
  InitializeParamsGetter,
} from "../../parameters_getter/params_getter_factory";
...
describe("CalculateEarnings()", () => {
  ...
  testCases.forEach((expectedResult, fileName) => {
    it(fileName, () => {
      ...
      InitializeParamsGetter(ParamsSource.File)
        .GetParams(filePath)
        .then((params) => {
          Calculator.CalculateEarnings(params).then((result) =>
