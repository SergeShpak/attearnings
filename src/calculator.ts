...
export class Calculator {
    static async CalculateEarnings(param: Parameters): Promise<BigInt> {
        ...
        await queue.construct(param.group, param.numberOfPlaces, param.numberOfRides);
        return new Promise<BigInt>((resolve, _) => {
            if (queue.Head === null) {
                return resolve(queue.StartTotal);
            }
            const fullCycles = Math.floor((param.numberOfRides - queue.StartLength) / queue.Length);
            const partialCycleLen = param.numberOfRides - queue.StartLength - fullCycles * queue.Length;
            return resolve(queue.StartTotal + queue.Total * BigInt(fullCycles) + queue.countPartialSum(partialCycleLen));
        });
...
class Queue {
    ...
    async construct(groups: AsyncIterator<number, null | Error, void>, placesCount: number, ridesCount: number) {
